...
| Project tab ×6 | 개요, 수집 범위, 프롬프트 통계, 블록/키워드 차트, 파생 지표, 0건 또는 D 한계, 원문 6건, 이름 붙은 기법 4–6개, 사례 2–3개, 시사점·한계 | 탭별 정량 감사 JSON + 원문 표본 SHA 검증 + 375/768/1280 캡처 |
| Author | 기존 연락처·방법론·읽기 가이드 유지 | 링크 4개와 탭 회귀 검증 |

- 8,000자는 반복을 허용하는 최소치가 아니라 A–F 증거층으로 채우는 목표다. 원문 아카이브가 없는 프로젝트는 허위 원문을 만들지 않고 D등급 대체 근거와 부족분을 수치로 밝힌다.
